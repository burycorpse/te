...
local function updateFov()
    local settings = getgenv().Silent.FovSettings
    clearFovParts()
...
local function isPartInFovAndVisible(part)
    ...
    return onScreen and distance <= getgenv().Silent.FovSettings.FovRadius
end

-- // Check if Part Visible // --
local function isPartVisible(part)
    if not getgenv().Silent.WallCheck then 
        return true
    end
...
local function GetVelocity(player, part)
    if player and player.Character then
        ...
        if velocity.Y < -30 and getgenv().Silent.Resolver then
            getgenv().Silent.Prediction = 0
            return velocity
        elseif velocity.Magnitude > 50 and getgenv().Silent.Resolver then
...
local function toggleFeature()
    ...
    local status = getgenv().Silent.Enabled and "Sharp Enabled" or "Sharp Disabled"
    sendNotification("Sentinel - Silent", status, "rbxassetid://119906580436119")
...
UserInputService.InputBegan:Connect(function(input, gameProcessed)
    if not gameProcessed and input.KeyCode == getKeyCodeFromString(getgenv().Silent.Keybind) then
        toggleFeature()
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Silent.Enabled then
        ...
        if Target and Target.Character then
            ...
            if closestPart and closestPoint then
                local hitPointSettings = getgenv().Silent.HitPoint
                ...
                if holdingMouseButton then
                    ...
                    Replicated_Storage.MainEvent:FireServer("UpdateMousePosI", closestPoint + velocity * getgenv().Silent.Prediction)
                end
            end
        else
            HitPoint.Visible = false
...
sendNotification(".gg/7mjx53xMYv", "Sentinel", "rbxassetid://119906580436119")
